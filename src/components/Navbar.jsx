...
const Navbar = () => {
    const {logo , menu_icon ,cross_icon}  = assets;
    const [ showMobileMenu , setShowMobileMenu] = useState(false);

    useEffect(()=>{
        if(showMobileMenu){
            document.body.style.overflow = 'hidden'//stop scrolling in small screen menu
  ...
  return (
    <div className="w-full top-0 left-0 absolute z-10">
        <div className="container mx-auto flex justify-between items-center  py-4 px-6 md:px-20 lg:px-32 bg-transparent"> 
            ...
            <img className="block md:hidden w-7 cursor-pointer" src={menu_icon}  onClick={()=>setShowMobileMenu(true)} />
        </div>
